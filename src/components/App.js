...
function reducer(state = states, action) {
   switch (action.type) {
      ...
      case 'cities': {
         return { ...state, cities: states[action.stateId].city };
      }
      case 'landmarks': {
         return { ...state, landmarks: state.cities[action.cityId].landmarks };
      }
      default: {
...
function App() {
   ...
   useEffect(() => {
      dispatch({ type: 'states' });
      dispatch({ type: 'cities', stateId: selectedState });
      dispatch({ type: 'landmarks', cityId: selectedCity });
   }, [selectedState, selectedCity, selectedLandMark]);
      ...
      <div id="main">
         ...
         <div className="displayData">
            <ul>
               <li id="state-name">
                  <h3 id="state-title">
                     {state.states.length && state.states[selectedState].name}
                  </h3>
                  <p id="state-description">
                     {state.states.length && state.states[selectedState].description}
                  </p>
               </li>
               <li id="city-name">
                  <h3 id="city-title">
                     {state.cities.length && state.cities[selectedCity].name}
                  </h3>
                  <p id="city-description">
                     {state.cities.length && state.cities[selectedCity].description}
                  </p>
               </li>
               <li id="landmark-name">
                  <h3 id="landmark-title">
                     {state.landmarks.length && state.landmarks[selectedLandMark].name}
                  </h3>
                  <p id="landmark-description">
                     {state.landmarks.length &&
                        state.landmarks[selectedLandMark].description}
                  </p>
               </li>
            </ul>
         </div>
